...
export default function ExportPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      while (true) {
        ...
        allPicks = [...allPicks, ...batch]
        if (batch.length < 1000) break
        from += 1000
      }
      setDraftPicks(allPicks.map(p => ({
        ...p,
  ...
  const datasets = useMemo(() => {
    // 1. Draft picks (raw)
    const draftPicksRows = draftPicks.map(p => ({
      season: p.season, round: p.round, pick_in_round: p.pick_in_round, overall_pick: p.overall_pick,
      manager: p.manager_name, player: p.player_name, position: p.position, nfl_team: p.nfl_team,
    }))

    // 2. Draft trends / history by manager
    const draftTrendsRows = [...new Set(draftPicks.map(p => p.manager_name))].sort().map(mgr => {
      const mp = draftPicks.filter(p => p.manager_name === mgr)
      const years = [...new Set(mp.map(p => p.season))].sort((a, b) => a - b)
      const row = { manager: mgr, seasons_drafted: years.length, years: years.join('|') }
      POSITIONS.forEach(pos => {
        ...
        row[`avg_round_first_${pos.replace('/', '')}`] = vals.length ? parseFloat((vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1)) : ''
      })
      const r1 = mp.filter(p => p.round === 1)
      const r1c = {}
      r1.forEach(p => { r1c[p.position] = (r1c[p.position] || 0) + 1 })
      row.most_common_round1_position = Object.entries(r1c).sort((a, b) => b[1] - a[1])[0]?.[0] || ''
      row.total_picks = mp.length
      return row
    })
    ...
    const managerCareerRows = managers.map(m => {
      ...
      return {
        ...
        championships, mol_bowls: molBowls, playoff_appearances: playoffAppearances,
    ...
    // 5. Yearly season standings (team-by-team, every season)
    ...
    // 6. Champions history (one row per season)
    const championsRows = seasons.map(s => {
      ...
      const molWinnerTeam = teams.find(t => t.season?.id === s.id && t.playoff_result === 'Mol Bowl Winner')
      return {
        year: s.year, champion: s.champion?.name || champTeam?.manager?.name || '',
        runner_up: runnerUpTeam?.manager?.name || '',
        mol_bowl_winner: molWinnerTeam?.manager?.name || '',
        mol_bowl_loser: s.mol_bowl_loser?.name || '',
      }
  ...
  const handleExport = () => {
    ...
    if (format === 'json') {
      ...
      downloadJSON(`fantasy-chatroom-export-${date}.json`, bundle)
    } else {
