 ...
 *
 * Running the season twice per game — once per winner — would cost two
 * simulations for every matchup. Instead each simulated season already picks a
 * winner for every tracked game, so bucketing that season's final standings by
 * those winners yields all the conditionals from a single pass.
 */
export function simulateLeverage({ rows, schedule = [], randomWeeks = 0, trackWeek = null, sims = 8000 }) {
  ...
  const tracked = []
  known.forEach((g, i) => { if (g.week === trackWeek) tracked.push(i) })

  const base = rows.map(blank)
  // cond[t][side][team] — side 0 is the home team winning, 1 the away team.
  ...
  for (let s = 0; s < sims; s++) {
    ...
    for (let k = 0; k < known.length; k++) {
      const homeWon = play(known[k].h, known[k].a)
      const t = tracked.indexOf(k)
      if (t >= 0) sideOf[t] = homeWon ? 0 : 1
    }
    ...
    if (n >= PLAYOFF_SPOTS) {
      ...
      p.forEach(i => { made[i] = true })
      p.slice(0, BYE_SPOTS).forEach(i => { gotBye[i] = true; inSemis[i] = true })
      const q1 = winnerOf(p[2], p[5])
      const q2 = winnerOf(p[3], p[4])
      inSemis[q1] = true; inSemis[q2] = true
    ...
    const record = (acc, t) => {
      acc.seedSum += seedOf[t]
      acc.winsSum += wins[t]
      if (made[t]) acc.playoffs++
      if (gotBye[t]) acc.bye++
      if (inSemis[t]) acc.semis++
      if (inFinals[t]) acc.finals++
      if (champ === t) acc.title++
    }

    for (let t = 0; t < n; t++) record(base[t], t)
    for (let g = 0; g < tracked.length; g++) {
      const bucket = cond[g][sideOf[g]]
      bucket.n++
      for (let t = 0; t < n; t++) record(bucket.teams[t], t)
    }
  }
  ...
  return {
    ...
    games: tracked.map((k, g) => {
      ...
      return {
        ...
        pHome: h.n / sims,
        // Null when a side never happened, which only occurs in tiny sim counts.
...
// season: MU is the median Week 1 leverage, so an opening-week game scores
// about 50, and SIGMA is set so a last-week win-and-in for both teams lands
// just under 100.
export const IMPORTANCE_MU = 0.409
export const IMPORTANCE_SIGMA = 0.682
