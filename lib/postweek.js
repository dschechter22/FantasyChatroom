import { buildRatings, makeLine, isPlayed } from './predictions'
 ...
 */
export function gradeWeek({ teams, matchups, rosterProj, week }) {
  const { byId } = buildRatings({ teams, matchups, throughWeek: week - 1, rosterProj })
  return matchups
 ...
 */
export function modelRecord({ teams, matchups, rosterProj, throughWeek }) {
  ...
  weeks.forEach(w => {
    // Week 1 has no prior results, so its lines are roster projection only.
    const games = gradeWeek({ teams, matchups, rosterProj, week: w })
    let favW = 0, favL = 0
    games.forEach(g => {
      rec.games++
