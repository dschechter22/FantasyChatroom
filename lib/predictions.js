...
const PRIOR_GAMES = 6        // shrinkage weight toward league average
...
// rating before any results exist. Decays as real games accumulate. Kept
// low -- a full-weight roster projection is one unshrunk number, and it
// otherwise pulls Week 1 ratings (and everything downstream: game lines,
// and especially the season simulation's futures markets, which compound
// even a mild single-game edge over an entire remaining season plus a
// bracket) much further apart than one week of evidence should justify.
const PROJ_WEIGHT = 0.2

// simulateFutures previously treated each team's shrunk `rating` as a known,
// fixed number across all 8000 trials, randomizing only game-to-game noise
// (sigma) around it. But the rating itself is an ESTIMATE, and after just a
// game or two it's a highly uncertain one (PRIOR_GAMES=6 means one real game
// gets only 1/7 weight) -- ignoring that estimation uncertainty is what made
// early-season futures (title/finals/semis/playoffs) look far more decided
// than one game of evidence justifies, even though the per-game ratings gap
// driving it was already fairly modest. RATING_UNCERTAINTY_MULT scales the
// posterior sd of the shrinkage estimate (leagueSd / sqrt(n + PRIOR_GAMES));
// tuned so a real Week 1 league (ratings ranging ~124-138, sigma ~23) lands
// on the title/playoff spread the numbers actually support -- top team
// around 14-15%, mid teams 11-12%, bottom teams 7-8% to win the title, and
// no team's playoff odds outside a realistic ~60-75% band with 6 of 10
// making it. It decays the same way shrinkage does, fading toward the plain
// per-game noise model as real results accumulate.
const RATING_UNCERTAINTY_MULT = 4
...
// Same best-available-lineup logic as projectedStarterPoints, but sourced
// from a specific week's uploaded per-player projection (roster_entries.stats.proj[week])
// rather than a season-long avg_pts. This is the real forward forecast
// buildRatings' weeklyProj expects — a genuine "here's what we think this
// team scores this week" number, not a season average.
export function projectedWeekScore(entries, week) {
  const byPos = { QB: [], RB: [], WR: [], TE: [], K: [], 'D/ST': [] }
  entries.forEach(e => {
    const pos = e.player?.position
    const proj = e.stats?.proj?.[week]
    if (byPos[pos] && proj != null) byPos[pos].push(proj)
  })
  Object.values(byPos).forEach(a => a.sort((x, y) => y - x))
  const take = (pos, n) => byPos[pos].splice(0, n)
  const starters = [...take('QB', 1), ...take('RB', 2), ...take('WR', 2), ...take('TE', 1), ...take('D/ST', 1), ...take('K', 1)]
...
// Same idea as projectedWeekScore but returns the actual lineup (starters +
// bench, each tagged with its slot and that week's projection) rather than
// just the total — what the matchup drawer renders per team.
export function projectedWeekLineup(entries, week) {
  const withProj = entries.map(e => ({ ...e, proj: e.stats?.proj?.[week] ?? null }))
  const byPos = { QB: [], RB: [], WR: [], TE: [], K: [], 'D/ST': [] }
  withProj.forEach(e => { if (byPos[e.player?.position]) byPos[e.player.position].push(e) })
  Object.values(byPos).forEach(a => a.sort((x, y) => (y.proj ?? -1) - (x.proj ?? -1)))
  ...
  const bench = withProj.filter(e => !usedIds.has(e.id)).sort((a, b) => (b.proj ?? -1) - (a.proj ?? -1))
  const total = starters.reduce((s, e) => s + (e.proj || 0), 0)
  return { starters, bench, total }
}

// Same optimal-lineup selection as projectedWeekLineup, but built from each
// player's actual score for the week (roster_entries.stats.actual[week])
// instead of a projection -- this is the "what should you have started"
// answer once the games are actually in, used by lineupEfficiency below.
export function actualWeekLineup(entries, week) {
  const withActual = entries.map(e => ({ ...e, actual: e.stats?.actual?.[week] ?? null }))
  const byPos = { QB: [], RB: [], WR: [], TE: [], K: [], 'D/ST': [] }
  withActual.forEach(e => { if (byPos[e.player?.position]) byPos[e.player.position].push(e) })
  Object.values(byPos).forEach(a => a.sort((x, y) => (y.actual ?? -1) - (x.actual ?? -1)))
  ...
  const flexPool = [...byPos.RB, ...byPos.WR, ...byPos.TE]
    .filter(e => !usedIds.has(e.id))
    .sort((a, b) => (b.actual ?? -1) - (a.actual ?? -1))
  if (flexPool[0]) { usedIds.add(flexPool[0].id); starters.push({ ...flexPool[0], slot: 'FLEX' }) }
  if (flexPool[1]) { usedIds.add(flexPool[1].id); starters.push({ ...flexPool[1], slot: 'FLEX' }) }

  const bench = withActual.filter(e => !usedIds.has(e.id)).sort((a, b) => (b.actual ?? -1) - (a.actual ?? -1))
  const total = starters.reduce((s, e) => s + (e.actual || 0), 0)
  return { starters, bench, total }
}

/**
 * "Did you start the right players" for one team-week, expressed as a
 * percentage: the points the manager's real starters scored, divided by
 * what the best possible starting lineup would have scored -- both read
 * from actual results (roster_entries.stats.actual[week]/.started[week]).
 * 100% means every start was the optimal call for that week.
 *
 * Returns null when there isn't enough actual-score data on file for the
 * week to trust the number (e.g. it hasn't been uploaded for most of the
 * roster yet), rather than a misleading number computed off a partial roster.
 */
export function lineupEfficiency(entries, week) {
  if (!entries.length) return null
  const withActual = entries.filter(e => e.stats?.actual?.[week] != null)
  if (withActual.length < entries.length * 0.8) return null

  const optimal = actualWeekLineup(entries, week)
  if (!optimal.total) return null

  const actualStarters = entries.filter(e => e.stats?.started?.[week] === true)
  const actualTotal = actualStarters.reduce((s, e) => s + (e.stats?.actual?.[week] || 0), 0)

  return {
    actualTotal: parseFloat(actualTotal.toFixed(2)),
    optimalTotal: parseFloat(optimal.total.toFixed(2)),
    pct: parseFloat(Math.min(100, (actualTotal / optimal.total) * 100).toFixed(1)),
  }
}
...
export function buildRatings({ teams, matchups, throughWeek, rosterProj = {}, baseline, weeklyProj = {} }) {
  ...
  const base = Object.values(td).map(t => {
    ...
    // How uncertain the `rating` point estimate itself still is, given how
    // little real data built it -- see RATING_UNCERTAINTY_MULT above. Only
    // simulateFutures uses this (per-trial season simulation); single-game
    // lines (makeLine) are unaffected.
    const ratingSD = RATING_UNCERTAINTY_MULT * leagueSd / Math.sqrt(n + PRIOR_GAMES)
    ...
    return {
      ...
      ratingSD: parseFloat(ratingSD.toFixed(2)),
    }
  })
...
export function simulateFutures({ rows, schedule = [], randomWeeks = 0, sims = 10000 }) {
  ...
  // seedTally[i][k] -- how many sims team i finished the regular season in
  // seed k (1-indexed). aheadCount[i][j] -- how many sims team i finished
  // with a better (lower) seed than team j. Both feed the sportsbook's
  // "over/under a seed" and "finish ahead of" futures markets.
  const seedTally = rows.map(() => new Array(n + 1).fill(0))
  const aheadCount = Array.from({ length: n }, () => new Array(n).fill(0))
  ...
  // This trial's belief about each team's true talent -- redrawn once per
  // trial from the rating's own uncertainty (see RATING_UNCERTAINTY_MULT),
  // not just game-to-game noise around a fixed number. See the note above
  // buildRatings' ratingSD for why.
  const trueRating = new Array(n)

  const play = (i, j) => {
    const si = gauss(trueRating[i], rows[i].sigma)
    const sj = gauss(trueRating[j], rows[j].sigma)
    pf[i] += si; pf[j] += sj
    if (si >= sj) { wins[i]++; return i }
    wins[j]++
    return j
  }
  const winnerOf = (i, j) => (gauss(trueRating[i], rows[i].sigma) >= gauss(trueRating[j], rows[j].sigma) ? i : j)

  for (let s = 0; s < sims; s++) {
    for (let i = 0; i < n; i++) {
      wins[i] = rows[i].wins; pf[i] = rows[i].pf
      trueRating[i] = rows[i].ratingSD ? gauss(rows[i].rating, rows[i].ratingSD) : rows[i].rating
    }
    ...
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < n; j++) {
        if (i !== j && rankOf[i] < rankOf[j]) aheadCount[i][j]++
      }
    }
  ...
  const out = rows.map((r, i) => {
    ...
    return {
      ...
      oddsOver,
      oddsUnder,
      // One entry per finishing seed (1..n) with its simulated probability --
      // the sportsbook's seed over/under market picks a line off this rather
      // than off avgSeed alone, since the mean seed is rarely a clean 50/50 split.
      seedProbs: seedTally[i].slice(1).map((cnt, k) => ({ seed: k + 1, p: cnt / sims })),
      // One entry per possible season win total with its simulated
      // probability -- lets a caller price a custom win-total line instead
      // of only the one auto-picked line above.
      winProbs: tally[i].map((cnt, w) => ({ wins: w, p: cnt / sims })).filter(x => x.p > 0),
      markets: [
        ...
        const [yes, no] = priceTwoWay(p)
        acc[k] = { p, odds: americanFromProb(p * 1.05), oddsYes: yes, oddsNo: no }
        return acc
      }, {}),
    }
  })
  // Probability team idA finishes with a better regular-season seed than
  // team idB -- the input for a "finishes ahead of" futures market. Attached
  // to the array rather than changing the return shape, so every existing
  // caller (which just treats this as a list of rows) keeps working.
  out.aheadProb = (idA, idB) => {
    const i = idx.get(idA), j = idx.get(idB)
    if (i == null || j == null) return null
    return aheadCount[i][j] / sims
  }
  return out
}
