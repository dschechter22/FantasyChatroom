 ...
 * game played through `throughWeek`, blended with roster makeup.
 */
export function buildRatings({ teams, matchups, throughWeek, rosterProj = {} }) {
  ...
  const leagueAvg = allScores.length ? mean(allScores) : 0
  const leagueVar = allScores.length > 1 ? variance(allScores) : FALLBACK_SD ** 2
  const leagueSd = Math.sqrt(leagueVar)

  const projValues = teams.map(t => rosterProj[t.id] || 0).filter(v => v > 0)
  const avgProj = projValues.length ? mean(projValues) : 0
  ...
  const base = Object.values(td).map(t => {
    ...
    // Roster makeup, rescaled onto the league's actual scoring level.
    const raw = rosterProj[t.team.id] || 0
    const rosterAnchor = raw > 0 && avgProj > 0
      ? (leagueAvg > 0 ? leagueAvg * (raw / avgProj) : raw)
      : null

    let rating
    if (n === 0) {
      rating = rosterAnchor ?? leagueAvg
    } else {
      ...
      rating = rosterAnchor != null
        ? 0.5 * shrunk + 0.25 * form + 0.25 * rosterAnchor
        : 0.65 * shrunk + 0.35 * form
    }
