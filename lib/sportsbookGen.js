...
import {
  ...
  projectedStarterPoints, projectedWeekScore, leagueBaseline,
} from './predictions'
...
// Straight passthrough of every REAL started player's own ESPN weekly
// projection -- no simulation, and no recomputed "optimal lineup" either
// (that used to run every team's roster through projectedWeekLineup's
// position-bucketing/FLEX logic to guess a starting 9, which only needs one
// mismatch -- an empty bucket, a slot nobody filled -- to quietly produce a
// prop for the QB and nothing else, exactly the "only QB props show up" bug
// this replaced). A player is either marked started for the week in ESPN's
// own data or they're not; trusting that directly is both simpler and
// exactly what the Scoreboard tab already shows, so the two can't disagree.
// Only ever adds a prop the first time a player is seen for a given week
// (never overwrites one that's already there, so an in-progress bet's line
// can't move under it).
export async function generateProps(db, { season, week, teams, rosterEntries }) {
  const errors = []
  const rows = []
  rosterEntries.forEach(e => {
    if (e.stats?.started?.[week] !== true) return
    const proj = e.stats?.proj?.[week]
    if (proj == null) return
    const teamMeta = teams.find(t => t.id === e.team_id)
    rows.push({
      season, week, player_id: e.player_id, player_name: e.player?.name || 'Unknown',
      position: e.player?.position || null, team_id: e.team_id, team_name: teamMeta?.manager?.name || teamMeta?.team_name || null,
      line: parseFloat(proj.toFixed(1)), odds_over: -110, odds_under: -110,
    })
  })
