...
function temperProb(p, weeksPlayed) {
...
export async function generateFutures(db, { season, week, teams, matchups, rosterEntries }) {
  ...
  const weeksPlayed = new Set(matchups.filter(m => (m.home_score ?? 0) > 0 || (m.away_score ?? 0) > 0).map(m => m.week)).size
  ...
  for (const r of sim) {
    ...
    ;['playoffs', 'bye', 'semis', 'finals', 'title'].forEach(key => {
      ...
      const fairP = temperProb(mkt.p, weeksPlayed)
      const [oddsYes, oddsNo] = priceTwoWay(fairP)
      rows.push({ season, market_type: key, team_id: r.id, opp_team_id: null, team_name: meta.name, opp_team_name: null, line: null, odds_yes: oddsYes, odds_no: oddsNo, fair_p: fairP })
    })
  }
...
export async function getOrCreateFuture(db, { season, marketType, teamId, oppTeamId = null, teamName, oppTeamName = null, line = null, fairP }) {
