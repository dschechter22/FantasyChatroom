...
//
// MU is the median raw leverage of an opening-week game, which fixes Week 1 at
// roughly 50. SIGMA is then solved so the extreme case — a final-week game both
// teams must win to reach the playoffs — lands at the target score below.
//
...
const rowsFrom = record => NAMES.map(n => ({
...
// ── Week 1: nothing played, every game is worth about the same.
const openers = simulateLeverage({
  rows: rowsFrom({}), schedule: fixtures, trackWeek: 1, sims: SIMS,
})
...
console.log('  median', median.toFixed(4), '· spread', (w1[w1.length - 1] - w1[0]).toFixed(4))
